/**
 * RaBbLE-NeBuLA.js — NeBuLA Renderer Module
 *
 * NeBuLA is the entity rendering substrate for the RaBbLE Collective.
 * This module owns all visual representation of the RaBbLE entity.
 *
 * Module identity:
 *   window.NeBuLA            — namespace; check this before calling renderer APIs
 *
 * Current backend: Canvas2D
 * Planned backends: Three.js (WebGL) → WebGPU → C++/Qt (desktop)
 *
 * Renderer API (all backends must implement):
 *   setEntityState(state)    — 'idle' | 'thinking' | 'speaking'
 *   injectEyeJolt(dx, dy)   — startle the entity (dx/dy in –1..1 range)
 *   resize()                 — recenter after host resize
 *   destroy()                — teardown
 *
 * Custom element:
 *   <rabble-entity mode="boot|idle" particle-count="480" overscan="2.55">
 *
 * Window events fired:
 *   rabble:entity-ready      — entity has fully rendered for the first time
 *   rabble:entity-state      — { detail: { state } } — every state change
 *
 * RaBbLE-entity.js is superseded by this file. Load this instead.
 */
(function (global) {
  'use strict';

  // ── NeBuLA module namespace ───────────────────────────────────────────────
  global.NeBuLA = {
    version:  '0.0.1',
    backend:  'Canvas2D',
    backends: ['Canvas2D'],  // future: 'Three.js', 'WebGPU', 'Qt/QML'

    // API contract — every backend must implement these on its instance
    api: ['setEntityState', 'injectEyeJolt', 'resize', 'destroy'],

    // Active renderer instance (set by the custom element on connectedCallback)
    _instance: null,

    setEntityState(state) {
      if (this._instance) this._instance.setEntityState(state);
    },
    injectEyeJolt(dx, dy) {
      if (this._instance) this._instance.injectEyeJolt(dx, dy);
    },
  };
  ...
  var NEBULA_RADIUS   = 130;
  var FALLOFF_RADIUS  = 155;
  ...
  function RaBbLEEntity(cv, opts) {
    ...
    function makeParticle(startAtTarget) {
      ...
      var r  = Math.pow(Math.random(), 0.52) * NEBULA_RADIUS;
      var tx = cx + r * Math.cos(a);
      var ty = cy + r * Math.sin(a) * 0.80;
      var sz = Math.random() * 11 + 0.8;
      return {
    ...
    function preserveParticlesOnResize() {
      var oldCx = cx, oldCy = cy;
      updateCentre();
      var dx = cx - oldCx, dy = cy - oldCy;
    ...
    if (opts.interactive) {
      ...
      onClick = function (e) {
        ...
        eyeVelX = 0; eyeVelY = 0;
        joltX = snapX / MAX_OFF; joltY = snapY / (MAX_OFF * 0.65);
        joltDecay = 1.0;
      };
    ...
    function updateEyeTarget() {
      ...
      if (t > holdUntil && t > nextSaccade) {
        ...
        var hd = kind === 'hard' ? 25 + Math.random() * 35 : kind === 'med' ? 45 + Math.random() * 55 : 65 + Math.random() * 80;
        var gd = kind === 'hard' ? 8  + Math.random() * 18 : kind === 'med' ? 20 + Math.random() * 40 : 40 + Math.random() * 70;
      ...
      if (t > nextDistraction) {
        var da = Math.random() * Math.PI * 2, reach = 0.45 + Math.random() * 0.55;
      ...
      var rawX, rawY;
      if (mouseNear) {
        var ang2 = Math.atan2(my, mx), str2 = Math.min(dist / 220, 1);
        var mouseSuggestX = Math.cos(ang2) * MAX_OFF * str2;
        var mouseSuggestY = Math.sin(ang2) * MAX_OFF * 0.65 * str2;
    ...
    function drawPortals(drawProgress) {
      ...
      var lpY = baseY + EYE_H * 0.44, rpY = baseY - EYE_H * 0.44;
      ...
      function portalArc(px, py, col, frac) {
        ...
        var glowBase = frac >= 1 ? (12 + 8 * Math.sin(t * 0.035 + (px - cx) * 0.05)) : (14 + 8 * Math.sin(t * 0.04));
        ctx.beginPath();
        var steps = Math.ceil(frac * 80) + 1;
        for (var s = 0; s <= steps; s++) {
          var ang3 = -Math.PI / 2 + (s / steps) * frac * Math.PI * 2;
          var ex = px + pW2 * Math.cos(ang3), ey = py + pH2 * Math.sin(ang3);
    ...
    function drawEyes(alpha, blinkScale, emergeScale) {
      ...
      var baseY = cy + BASE_Y_OFFSET;
      var lx = cx - EYE_GAP, rx = cx + EYE_GAP;
      var lPortalY = baseY + EYE_H * 0.44, rPortalY = baseY - EYE_H * 0.44;
      ...
      var lOrbX = lRestX, lOrbY = lPortalY + (lRestY - lPortalY) * easeInOut(emergeScale);
      var rOrbX = rRestX, rOrbY = rPortalY + (rRestY - rPortalY) * easeInOut(emergeScale);
    ...
    function frame() {
      ...
      parts.forEach(function (p) {
        ...
        var falloff = Math.max(0, 1 - d / FALLOFF_RADIUS);
      ...
      if (settleBlend > 0) {
        var connAlpha = settleBlend * 0.13, connDist = 62 + settleBlend * 20;
        for (var i = 0; i < parts.length; i += 2) {
          ...
          for (var j = i + 2; j < parts.length; j += 2) {
            ...
            var ddx = (pi.x + dxi) - (pj.x + dxj), ddy = (pi.y + dyi) - (pj.y + dyj);
    ...
    // ── Public API ────────────────────────────────────────────────────────
    this.setEntityState = function (s) {
      entityState = s;
      global.dispatchEvent(new CustomEvent('rabble:entity-state', { detail: { state: s } }));
    };
  ...
  if (typeof HTMLElement !== 'undefined') {
    class RaBbLEEntityElement extends HTMLElement {
      ...
      disconnectedCallback() {
        ...
        if (global.NeBuLA._instance === this._entity) global.NeBuLA._instance = null;
